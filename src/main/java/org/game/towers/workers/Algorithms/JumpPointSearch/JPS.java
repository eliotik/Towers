...
public class JPS {
    ...
    private volatile Node tmpNode;
//    private volatile Node[] successors, possibleSuccess;
    private volatile Node[] successors, possibleSuccess;
    private volatile ArrayList<Node> trail;
    private HashMap<Integer, Grid> gridHashMap = new HashMap<Integer, Grid>();
    private HashMap<Integer, Node[]> successorsHashMap = new HashMap<Integer, Node[]>();
    private HashMap<Integer, Node[]> possibleSuccessHashMap = new HashMap<Integer, Node[]>();
...
//            possibleSuccess = identifySuccessors(cur, unitId);
	...
	public synchronized Node[] identifySuccessors(Node node){
	...
	public synchronized int[] jump(int x, int y, int px, int py){
		int[] jx = {-1,-1}; //used to later check if full or null
		int[] jy = {-1,-1}; //used to later check if full or null
		int dx = (x-px)/Math.max(Math.abs(x-px), 1); //because parents aren't always adjacent, this is used to find parent -> child direction (for x)
		int dy = (y-py)/Math.max(Math.abs(y-py), 1); //because parents aren't always adjacent, this is used to find parent -> child direction (for y)
	 ...
	 */
	public synchronized int[][] getNeighborsPrune(Node node){
